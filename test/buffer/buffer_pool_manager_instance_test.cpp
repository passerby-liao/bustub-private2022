...
//   //LOG_INFO("test data %s",random_binary_data);
...
//     //LOG_INFO("60   page   0   is nullptr  data _    %s\n",bpm->FetchPage(0)->GetData());

//   // Scenario: We should be able to create new pages until we fill up the buffer pool.
//   for (size_t i = 1; i < buffer_pool_size; ++i) {
//     EXPECT_NE(nullptr, bpm->NewPage(&page_id_temp));
//     //LOG_INFO("66   i:%ld  page   id  %d \n",i,page_id_temp);
//   }
...
//   for (int i = 0; i < 5; ++i) {
//     EXPECT_NE(nullptr, bpm->NewPage(&page_id_temp));
//     //LOG_INFO("82   i:%d  page   id  %d \n",i,page_id_temp);
//     bpm->UnpinPage(page_id_temp, false);
//     //     if(i==3)
//     // {
//     //   LOG_INFO("83   i:%d  page   0   is  data _      %s\n",i,bpm->FetchPage(0)->GetData());
//     // }
//   }

//   // Scenario: We should be able to fetch the data we wrote a while ago.
//   page0 = bpm->FetchPage(0);
//   //if(page0)  {LOG_INFO("85 test data %s",page0->GetData());}
//   std::memcpy(page0->GetData(), random_binary_data, BUSTUB_PAGE_SIZE);
...
//   delete disk_manager;
// }
// TEST(BufferPoolManagerInstanceTest2, SampleTest) {
//   const std::string db_name = "test.db";
//   const size_t buffer_pool_size = 10;
//   const size_t k = 5;
...
//   delete disk_manager;
// }
// TEST(aaa, BinaryDataTest) {  // NOLINT
//   const std::string db_name = "test.db";
//   const size_t buffer_pool_size = 10;
//   const size_t k = 5;
...
// TEST(ggg, ConcurrencyTest) {  // NOLINT
...
// TEST(hhh, IntegratedTest) {  // NOLINT
//   page_id_t temp_page_id;
//   auto *disk_manager = new DiskManager("test.db");
//   auto *bpm = new BufferPoolManagerInstance(10, disk_manager, 5);
...
//           } else {
//             ASSERT_EQ(0, std::strcmp((std::string("Hard") + std::to_string(page_ids[j])).c_str(),
//             (page->GetData()))); ASSERT_EQ(1, bpm->UnpinPage(page_ids[j], false, nullptr));
//           }
//           j = (j + 1);
//         }
//       }));
...
TEST(kkk, HardTest_3) {       // NOLINT
  const int num_threads = 5;  // 5
  const int num_runs = 50;    // 10
  for (int run = 0; run < num_runs; run++) {
    auto *disk_manager = new DiskManager("test.db");
    std::shared_ptr<BufferPoolManagerInstance> bpm{new BufferPoolManagerInstance(50, disk_manager)};
    std::vector<std::thread> threads;
