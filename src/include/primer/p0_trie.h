...
class TrieNode {
 public:
  ...
  TrieNode(TrieNode &&other_trie_node) noexcept {
    ...
    // this->children_ = other_trie_node.children_;
  ...
  void RemoveChildNode(char key_char) {
    ...
    this->children_.erase(this->children_.find(key_char));
  }
...
class Trie {
 ...
 public:
  ...
  bool Insert(const std::string &key, T value) {
    ...
    latch_.WLock();
    std::unique_ptr<TrieNode> *cur = &this->root_;
    int len = key.size();
    ...
    for (int i = 0; i < len; i++) {
      ...
      if (!(*cur)->HasChild(x)) {
        if (i != len - 1) {
          std::unique_ptr<TrieNode> child(new TrieNode(x));
          cur = cur->get()->InsertChildNode(x, std::move(child));
        } else {
          std::unique_ptr<TrieNodeWithValue<T>> child(new TrieNodeWithValue(x, value));
          cur = cur->get()->InsertChildNode(x, std::move(child));
          // LOG_INFO("Insert endnode %s %d", key.c_str(), cur->get()->IsEndNode());
        }
      } else {
        if (i == len - 1) {
          std::unique_ptr<TrieNode> *next = cur->get()->GetChildNode(x);
          if (next->get()->IsEndNode()) {
            latch_.WUnlock();
            return false;
          }
          std::unique_ptr<TrieNodeWithValue<T>> child(new TrieNodeWithValue<T>(std::move(*(next->get())), value));
          //          std::unique_ptr<TrieNodeWithValue<T>>
          //          child=std::make_unique<TrieNodeWithValue<T>>(std::move(*(next->get())),value);
          cur->get()->RemoveChildNode(x);
          cur = cur->get()->InsertChildNode(x, std::move(child));
          // LOG_INFO("Insert change %s %d", key.c_str(), cur->get()->IsEndNode());
          latch_.WUnlock();
          return true;
        }
  ...
  bool RemoveRecursive(std::unique_ptr<TrieNode> *cur, const std::string &key, int dex) {
    if (dex == static_cast<int>(key.size())) {
      return true;
    }
    if (RemoveRecursive(cur->get()->GetChildNode(key[dex]), key, dex + 1)) {
      cur->get()->RemoveChildNode(key[dex]);
      // LOG_INFO(" RemoveRecursive %s dex:%d is deleted  now value %d", key.c_str(), dex+1,);
      if (!cur->get()->HasChildren() && !cur->get()->IsEndNode()) {
        return true;
      }
    }
    // LOG_INFO(" RemoveRecursive %s dex:%d is end", key.c_str(), dex);
    return false;
  }
  ...
  bool Remove(const std::string &key) {
    ...
    if (!cur->get()->IsEndNode()) {
      // LOG_INFO(" IsEndNode return false %s", key.c_str());
      latch_.WUnlock();
      return false;
    }
    if (cur->get()->HasChildren()) {
      cur->get()->SetEndNode(false);
    } else {
      // LOG_INFO(" delete %s", key.c_str());
      this->RemoveRecursive(&this->root_, key, 0);
    }
    latch_.WUnlock();
    return true;
  }
  ...
  T GetValue(const std::string &key, bool *success) {
    ...
    *success = true;
    auto *nodewithvalue = dynamic_cast<TrieNodeWithValue<T> *>(cur->get());
    if (!nodewithvalue) {
      latch_.RUnlock();
      *success = false;
      return {};
