  ...
  while (child_->Next(&left_tuple, &emit_rid)) {
    ...
    if (!rids.empty()) {
      ...
      for (uint32_t idx = 0; idx < child_->GetOutputSchema().GetColumnCount(); idx++) {
        vals.emplace_back(left_tuple.GetValue(&child_->GetOutputSchema(), idx));
      }
      for (uint32_t idx = 0; idx < plan_->InnerTableSchema().GetColumnCount(); idx++) {
        vals.emplace_back(right_tuple.GetValue(&plan_->InnerTableSchema(), idx));
      }
    ...
    if (plan_->GetJoinType() == JoinType::LEFT) {
      ...
      for (uint32_t idx = 0; idx < plan_->InnerTableSchema().GetColumnCount(); idx++) {
        vals.emplace_back(ValueFactory::GetNullValueByType(plan_->InnerTableSchema().GetColumn(idx).GetType()));
      }
      *tuple = Tuple(vals, &GetOutputSchema());
      return true;
    }
