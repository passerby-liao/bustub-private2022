...
auto B_PLUS_TREE_LEAF_PAGE_TYPE::Insert(const KeyType &key, const ValueType &value, const KeyComparator &comparator)
    -> int {
  auto distance_in_array = KeyIndex(key, comparator);
  if (distance_in_array == GetSize()) {
    *(array_ + distance_in_array) = std::make_pair(key, value);
    IncreaseSize(1);
    return GetSize();
  }
  if (comparator(array_[distance_in_array].first, key) == 0) {
    return GetSize();
  }
...
void B_PLUS_TREE_LEAF_PAGE_TYPE::MoveHalfTo(BPlusTreeLeafPage *recipient) {
  ...
  SetSize(split_index);
  recipient->CopyNFrom(array_ + split_index, GetMaxSize() - split_index);
}
...
INDEX_TEMPLATE_ARGUMENTS
auto B_PLUS_TREE_LEAF_PAGE_TYPE::LookupOptimistic(const KeyType &key, const KeyComparator &comparator) const -> bool {
  auto target_in_array = KeyIndex(key, comparator);

  return (target_in_array != GetSize() && comparator(key, array_[target_in_array].first) == 0);
}
...
auto B_PLUS_TREE_LEAF_PAGE_TYPE::Lookup(const KeyType &key, ValueType *value, const KeyComparator &comparator) const
  ...
  if (target_in_array == GetSize() || comparator(key, array_[target_in_array].first) != 0) {
    return false;
  }
...
auto B_PLUS_TREE_LEAF_PAGE_TYPE::RemoveAndDeleteRecord(const KeyType &key, const KeyComparator &comparator) -> int {
  int target_in_array = KeyIndex(key, comparator);
  if (target_in_array == GetSize() || comparator(array_[target_in_array].first, key) != 0) {
    return GetSize();
  }
